...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // articles prices manufacturers
  var articlesPricesManufacturersTrigger = document.querySelectorAll('.articles-prices-manufacturers__trigger');
  if (articlesPricesManufacturersTrigger) {
    articlesPricesManufacturersTrigger.forEach(function (item) {
      var close = function close() {
        var parent = item.closest('.articles-prices-manufacturers__top');
        parent.classList.remove('is--active');
      };
      var itemChecker = useItemChecker([item.parentNode], close);
      item.addEventListener('click', function (event) {
        ...
        setTimeout(function () {
          ...
          if (bigReviewsContent) {
            bigReviewsContent.forEach(function (content) {
              var currentFullHeight = content.scrollHeight;
              content.style.setProperty('--full-height', currentFullHeight + 'px');
              if (window.innerWidth >= 768) {
                if (currentFullHeight >= 27 * 5) {
